...
func NewRestApi(conf *config.Config) *RestApi {
	if restApi == nil {
		restApi = &RestApi{
			Address:   conf.Api.Listen,
			routerMap: map[string]RouteOption{},
			Auth:      conf.Auth.Jwt,
...
func (rest *RestApi) AuthRouteSet(path string) *RestSet {
	res := RestSet{Path: path, Auth: rest.Auth, SecretKey: rest.SecretKey}
	return &res
}
...
func (r *RestSet) addToRouterMap(method string, name string, handler func(ct *Context) (interface{}, *Error)) {
	...
	if _, exist := restApi.routerMap[key]; !exist {
		restApi.routerMap[key] = RouteOption{Method: method, Handler: handler, Auth: r.Auth, secretKey: r.SecretKey}
	} else {
...
func (rp *RouteOption) Handle(ct *Context) (interface{}, *Error) {
	if rp.Auth {
		...
		if len(authorizations) > 1 {
			if !validateToken(authorizations[1], rp.secretKey) {
